...
cd ~
sudo git clone https://github.com/etnpoolorg/nodejs-pool.git  # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd electroneum
sudo git checkout
sudo curl https://raw.githubusercontent.com/etnpoolorg/nodejs-pool/master/deployment/electroneum_daemon.patch | sudo git apply -v
sudo cmake .
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/electroneumtestnet.service /lib/systemd/system/
...
mysql -u root --password=$ROOT_SQL_PASS testnet -e "INSERT INTO testnet.config (module, item, item_value, item_type, Item_desc) VALUES ('api', 'authKey', '`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1`', 'string', 'Auth key sent with all Websocket frames for validation.')"
mysql -u root --password=$ROOT_SQL_PASS testnet -e "INSERT INTO testnet.config (module, item, item_value, item_type, Item_desc) VALUES ('api', 'secKey', '`cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1`', 'string', 'HMAC key for Passwords.  JWT Secret Key.  Changing this will invalidate all current logins.')"
